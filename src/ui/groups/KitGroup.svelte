<script lang="ts">
  /** Kit-level bus: the kit's own filters, effects and sidechain, applied after every row. */
...
<div class="fields">
  ...
  {/if}
  <Select label="Mod FX" name="kit.modFXType" value={kit.attrs.modFXType} options={modFxOptions(editor.supports)} onchange={set('modFXType')} />
</div>
<div class="knobrow">
  ...
  <HexKnob el={params} ensure={P} attr="reverbAmount" label="Reverb" order={KIT_PARAM_ATTRS} />
  ...
  <HexKnob el={dly} ensure={() => ensureChild(P(), 'delay', KIT_PARAMS_CHILD_ORDER)} attr="rate" label="Delay Time" order={RATE} />
  <HexKnob el={dly} ensure={() => ensureChild(P(), 'delay', KIT_PARAMS_CHILD_ORDER)} attr="feedback" label="Delay Fb" order={RATE} />
  <HexKnob el={params} ensure={P} attr="modFXDepth" label="FX Depth" order={KIT_PARAM_ATTRS} />
  <HexKnob el={params} ensure={P} attr="modFXRate" label="FX Rate" order={KIT_PARAM_ATTRS} />
...
<div class="fields">
  <Select label="Delay Sync" name="kit.delay.syncLevel" value={delay?.attrs.syncLevel} options={syncLevelOptions()} onchange={(v) => setAttr(ensureChild(kit, 'delay', KIT_CHILD_ORDER), 'syncLevel', v, DELAY_ATTR_ORDER)} />
  {#if editor.supports('syncType')}
    <Select label="Sync Type" name="kit.delay.syncType" value={delay?.attrs.syncType} options={syncTypeOptions()} onchange={(v) => setAttr(ensureChild(kit, 'delay', KIT_CHILD_ORDER), 'syncType', v, DELAY_ATTR_ORDER)} />
  {/if}
  <div class="f"><span class="lbl">Stereo</span><Toggle label="Ping-pong" name="kit.delay.pingPong" value={delay?.attrs.pingPong} onchange={(v) => setAttr(ensureChild(kit, 'delay', KIT_CHILD_ORDER), 'pingPong', v, DELAY_ATTR_ORDER)} /></div>
</div>
