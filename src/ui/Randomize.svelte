...
{#if randomizer.open}
  <section class="gen" data-testid="randomize-panel" aria-label="Patch generator">
    <div class="grp">
      ...
      <div class="seg" data-testid="randomize-intensity">
    ...
    <p class="note">
      {#if randomizer.target}
        ...
        <span data-testid="randomize-target">Rolls <b>{randomizer.target}</b> ·</span>
      {/if}
      A roll is edits like any other — <b>Changes</b> lists every one, and each can be put back. Nothing reaches the Deluge until you save.
    </p>
...
<style>
   ...
   */
  .gen {
    display: flex; flex-wrap: wrap; align-items: center; gap: 8px 16px; position: relative;
  ...
  .seg button.on { border-color: var(--brass); color: var(--brass-hi); background: #241d13; }
  .chk {
  ...
  .go { gap: 6px; }
  .btn {
    height: 25px; padding: 0 14px; border: 1px solid var(--edge-hi); background: #1b1815; color: var(--muted);
  ...
  .x { position: absolute; top: 6px; right: 8px; background: none; border: 0; color: var(--faint); font-size: 15px; line-height: 1; cursor: pointer; }
