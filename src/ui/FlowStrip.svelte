<script lang="ts">
  ...
  function clickBlock(e: MouseEvent, g: Group) {
    e.stopPropagation()
    editor.toggleFocus(g.id, e.shiftKey)
  }
...
<div class="rig" bind:this={rig} onclick={() => editor.clearFocus()} onkeydown={(e) => { if (e.key === 'Escape') editor.clearFocus() }} data-testid="flow-strip" title="Click a block to focus it; shift-click to pin several; click the background to show everything">
...
<style>
  ...
  .rigrow { display: flex; align-items: center; gap: 0; position: relative; z-index: 2; min-width: max-content; }
  .cap { flex: none; width: 52px; height: 56px; border-radius: 6px; border: 1px solid var(--edge-hi); background: #141210; display: grid; place-items: center; font-family: var(--cond); font-size: 11px; font-weight: 700; letter-spacing: .12em; text-transform: uppercase; color: var(--muted); line-height: 1.1; text-align: center; }
  .cap.out { border-color: var(--brass-dim); color: var(--brass); }
  .link { flex: none; width: 18px; height: 2px; background: #2e2820; }
  ...
  .modgroups { display: flex; gap: 0; }
  .mods { display: flex; flex-wrap: wrap; gap: 6px; flex: 1; min-width: 240px; padding-top: 4px; }
